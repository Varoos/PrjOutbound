...
function StockUpdation() {
    debugger
    Focus8WAPI.getFieldValue("StockUpdationCallback", ["", "DocNo"], Focus8WAPI.ENUMS.MODULE_TYPE.TRANSACTION, false, requestId);
}

function StockUpdationCallback(response) {
    debugger
    console.log(response);
    logDetails = response.data[0];
    docNo = response.data[1].FieldValue;
    $.ajax({
        url: baseUrl + "/Transaction/StockUpdation",
        type: "POST",
        datatype: 'JSON',
        data: { "CompanyId": logDetails.CompanyId, "SessionId": logDetails.SessionId, "LoginId": logDetails.LoginId, "vtype": logDetails.iVoucherType, "DocNo": docNo },
